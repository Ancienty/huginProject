...
public class XmlExportGUI {
    ...
    public XmlExportGUI(Database database, Runnable returnToMain) {
        this.database = database;
        this.returnToMain = returnToMain;
        this.frame = new JFrame("XML Dışa Aktarma");
        initializeGUI();
    }
    ...
    private void initializeGUI() {
        ...
        JLabel titleLabel = new JLabel("XML Dışa Aktarma", SwingConstants.CENTER);
        ...
        btnExport = createStyledButton("Kaydet", SMALL_BUTTON_WIDTH, SMALL_BUTTON_HEIGHT);
        btnExport.setEnabled(false);
        btnExport.addActionListener(e -> exportToXml());

        JButton btnRefresh = createStyledButton("Yenile", SMALL_BUTTON_WIDTH, SMALL_BUTTON_HEIGHT);
        btnRefresh.addActionListener(e -> refreshInvoiceList());

        JButton btnCancel = createStyledButton("Geri", SMALL_BUTTON_WIDTH, SMALL_BUTTON_HEIGHT);
    ...
    private JPanel createInvoiceSelectionPanel() {
        ...
        TitledBorder border = BorderFactory.createTitledBorder("Fatura Seçimi");
        ...
        int[] widths = {40, 60, 80, 150, 80, 100};
        ...
        invoiceTable.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2 && btnExport.isEnabled()) {
                    exportToXml();
                }
            }
        });
        ...
        scroll.setPreferredSize(TABLE_SCROLL_SIZE);
        panel.add(scroll, BorderLayout.CENTER);

        JLabel tip = new JLabel("<html><i>Çift tıklayarak hızlı kaydet, önizleme için seçin</i></html>");
    ...
    private JPanel createPreviewPanel() {
        JPanel panel = new JPanel(new BorderLayout());
        TitledBorder border = BorderFactory.createTitledBorder("XML Önizleme");
        panel.setBorder(BorderFactory.createCompoundBorder(
                border,
                new EmptyBorder(15, 15, 15, 15)
        ));
        ...
        txtPreview.setText("Fatura seçerek XML önizleme görebilirsiniz...");
        txtPreview.setBorder(new EmptyBorder(10, 10, 10, 10));

        JScrollPane scroll = new JScrollPane(txtPreview);
        scroll.setPreferredSize(PREVIEW_SCROLL_SIZE);
    ...
    private void updatePreview() {
        ...
        try {
            ...
            if (inv == null) throw new RuntimeException("Yok");
            String xml = createXmlContent(inv);
            txtPreview.setText(xml);
    ...
    private String createXmlContent(Database.Invoice invoice) {
        try {
            InvoiceUploadData.UploadSystem uploadSystem = new InvoiceUploadData.UploadSystem();
            ...
            uploadSystem.customer = customer;
            ...
            List<Database.InvoiceItem> dbItems = database.getInvoiceItems(invoice.id);
            invoiceData.items = new java.util.ArrayList<>();
            for (Database.InvoiceItem dbItem : dbItems) {
                InvoiceUploadData.Item item = new InvoiceUploadData.Item();
                item.name = dbItem.name;
                item.quantity = dbItem.quantity;
                item.unitPrice = dbItem.unitPrice;
                item.lineTotal = dbItem.lineTotal;
                invoiceData.items.add(item);
            }
            uploadSystem.invoiceData = invoiceData;

            XmlMapper xmlMapper = new XmlMapper();
            return xmlMapper.writerWithDefaultPrettyPrinter().writeValueAsString(uploadSystem);

        } catch (Exception e) {
            System.err.println("Error creating XML content: " + e.getMessage());
            e.printStackTrace();
    ...
    private void exportToXml() {
        ...
        try {
            ...
            chooser.setDialogTitle("XML Dosyasını Kaydet");
            String name = String.format("fatura_%s_%s_%s.xml",
                    invoice.series,
                    invoice.number,
            ...
            if (chooser.showSaveDialog(frame) == JFileChooser.APPROVE_OPTION) {
                ...
                try (FileWriter writer = new FileWriter(file)) {
                    writer.write(createXmlContent(invoice));
                }
                JOptionPane.showMessageDialog(frame,
                        "XML dosyası başarıyla kaydedildi:\n" + file.getAbsolutePath(),
                        "Başarılı",
                        JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(frame,
                    "XML dışa aktarılırken hata oluştu:\n" + e.getMessage(),
                    "Hata",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
